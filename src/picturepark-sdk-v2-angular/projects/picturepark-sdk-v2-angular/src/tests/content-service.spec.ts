...
describe('ContentService', () => {
  ...
  it(
    'should download content thumbnail',
    waitForAsync(
      inject([ContentService], async (contentService: ContentService) => {
        // arrange
  ...
  it(
    'should download resized content',
    waitForAsync(
      inject([ContentService], async (contentService: ContentService) => {
        // arrange
        ...
        const response = await contentService.search(request).toPromise();
        const result = await contentService
          .download(response?.results[0].id ?? '', 'Original', 100, 100, null)
          .toPromise();
  ...
  it(
    'should return some aggregations',
    waitForAsync(
      inject([ContentService], async (contentService: ContentService) => {
        // arrange
