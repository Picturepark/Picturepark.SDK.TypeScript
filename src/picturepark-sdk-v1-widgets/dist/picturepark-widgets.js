var pictureparkWidgets = (function () {
  ...
  define("players", ["require", "exports"], function (require, exports) {
      ...
      var PictureparkPlayers = (function () {
          ...
          PictureparkPlayers.disposeVideoPlayer = function (player) {
              var existingPlayer = PictureparkPlayers.loadedPlayers.filter(function (p) { return p.player === player; })[0];
              if (existingPlayer) {
                  player.dispose();
                  PictureparkPlayers.loadedPlayers = PictureparkPlayers.loadedPlayers
                      .filter(function (p) { return p.player !== player; });
              }
              console.log('Picturepark Widgets > Disposed videojs player');
          };
          PictureparkPlayers.renderVideoPlayerIfNeeded = function (item, element, width, height) {
              var playerInfo = PictureparkPlayers.loadedPlayers.filter(function (p) { return p.element === element.id; })[0];
              if (playerInfo) {
                  return playerInfo.promise.then(function (player) {
                      var element = document.getElementById(playerInfo.element);
                      if (!element || !element.tagName || element.tagName.toLowerCase() === 'video') {
                          if (player) {
                              PictureparkPlayers.disposeVideoPlayer(player);
                          }
                          return PictureparkPlayers.renderVideoPlayer(element, item, width, height).then(function (player) {
              ...
              PictureparkPlayers.loadedPlayers = PictureparkPlayers.loadedPlayers.filter(function (p) { return p.element !== element.id; });
              playerInfo = {
                  element: element.id,
                  promise: PictureparkPlayers.renderVideoPlayer(element, item, width, height).then(function (player) {
          ...
          PictureparkPlayers.renderVideoPlayer = function (element, item, width, height) {
              return this.loadVideoPlayerLibraries().then(function (videojs) {
          ...
          PictureparkPlayers.loadVideoPlayerLibraries = function () {
              if (window.videojs)
                  ...
                  if (shareItems.filter(function (i) { return i.isMovie || i.isPdf; }).length > 0) {
                      var updatePlayers = function () {
                          if (shareItems.filter(function (i) { return i.isMovie; }).length > 0) {
                              PictureparkPlayers.loadVideoPlayerLibraries().then(function () {
                                  for (var _i = 0, _a = shareItems.filter(function (i) { return i.isMovie; }); _i < _a.length; _i++) {
                                      var item = _a[_i];
                                      var elementId = "vjsplayer_" + item.id;
                                      var element = document.getElementById(elementId);
                                      if (element) {
                                          PictureparkPlayers.renderVideoPlayerIfNeeded(item, element, window.innerWidth, window.innerHeight).then(function (player) {
                                              if (player) {
                  ...
                  return new Promise(function (resolve) {
                      photoSwipe.listen('close', function () {
                          for (var _i = 0, loadedPlayers_1 = loadedPlayers; _i < loadedPlayers_1.length; _i++) {
                              ...
                              _this.disposeVideoPlayer(player);
                          }
  ...
  /// <reference path="../../picturepark-sdk-v1-fetch/dist/picturepark.d.ts" />
  define("index", ["require", "exports", "templates", "players", "rendering", "config", "players", "libraries/promise.min", "libraries/liquid.min", "./libraries/fetch.min.js"], function (require, exports, templates_1, players_1, rendering_1, config_1, players_2) {
      "use strict";
      exports.__esModule = true;
      exports.players = players_2.PictureparkPlayers;
      /**
      ...
      function processScriptTag(scriptTag) {
          ...
          return window.fetch(initialConfig.server + '/json/' + initialConfig.token).then(function (response) {
              ...
              var share = {
                  ...
                  items: shareDetail.contentSelections.map(function (s) {
                      ...
                      return {
                          ...
                          isMovie: originalOutput ? players_1.PictureparkPlayers.videoExtensions.indexOf(originalOutput.fileExtension) !== -1 : false,
                          isImage: originalOutput ? players_1.PictureparkPlayers.imageExtensions.indexOf(originalOutput.fileExtension) !== -1 : false,
              ...
              return engine.parseAndRender(contentTemplate, {
                  ...
                  var _loop_3 = function (item) {
                      if (item.isMovie) {
                          ...
                          setTimeout(function () {
                              var element = document.getElementById(elementId_1);
                              if (element) {
                                  players_1.PictureparkPlayers.renderVideoPlayerIfNeeded(item, element, config.width, config.height).then(function (player) {
                                      share.player = player;
                                  });
                              }
                          });
      ...
      if (players_1.PictureparkPlayers.scriptsPath === undefined)
          players_1.PictureparkPlayers.scriptsPath = getScriptsPath();
