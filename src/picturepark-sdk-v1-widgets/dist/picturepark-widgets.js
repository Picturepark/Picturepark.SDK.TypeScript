var pictureparkWidgets = (function () {
  ...
  define("players", ["require", "exports"], function (require, exports) {
      ...
      var PictureparkPlayers = (function () {
          ...
          PictureparkPlayers.showDetail = function (token, itemId, widgetId) {
              if (PictureparkPlayers.loading)
                  return;
              PictureparkPlayers.loading = true;
              var share = document.pictureparkShareCache[token];
              var item = share.items.filter(function (i) { return i.id === itemId; })[0];
              if (item.isPdf && share.items.length === 1) {
                  ...
                  this.showPhotoSwipeItem(token, item, share.items, 'gallery_' + widgetId).then(function () {
                      PictureparkPlayers.loading = false;
          ...
          PictureparkPlayers.renderVideoPlayer = function (token, id, elementId, width, height) {
              var share = document.pictureparkShareCache[token];
              var item = share.items.filter(function (i) { return i.id === id; })[0];
              return this.loadVideoPlayer().then(function (jwplayer) {
                  var player = jwplayer(elementId);
                  if (player.setup) {
          ...
          PictureparkPlayers.showPhotoSwipeItem = function (token, item, items, galleryElementId) {
              return this.loadPhotoSwipe().then(function (result) {
                  var photoSwipeItems = items.map(function (i) {
                      if (i.isImage) {
                          return {
                              src: i.originalUrl,
                              w: i.detail.width,
                  ...
                  gallery.listen('afterChange', function () {
                      var g = PictureparkPlayers.getGallery(galleryElementId);
                      PictureparkPlayers.showGalleryItem(g, gallery.getCurrentIndex());
                  });
                  var players = [];
                  var resizeCallbacks = [];
                  if (items.filter(function (i) { return i.isMovie || i.isPdf; }).length > 0) {
                      var updatePlayers = function () {
                          if (items.filter(function (i) { return i.isMovie; }).length > 0) {
                              // TODO: Only update if not already initialized
                              PictureparkPlayers.loadVideoPlayer().then(function () {
                                  for (var _i = 0, _a = items.filter(function (i) { return i.isMovie; }); _i < _a.length; _i++) {
                                      var i = _a[_i];
                                      PictureparkPlayers.renderVideoPlayer(token, i.id, "jwplayer_" + i.id, window.innerWidth, window.innerHeight).then(function (player) {
                                          if (player) {
                                              players.push(player);
                                              var resizeCallback = function () { return player.resize(window.innerWidth, window.innerHeight); };
                          ...
                          for (var _i = 0, _a = items.filter(function (i) { return i.isPdf; }); _i < _a.length; _i++) {
                              var i = _a[_i];
                              _loop_1(i);
                          }
                      };
          ...
          PictureparkPlayers.loadScript = function (url, useRequire, globalName) {
              if (useRequire && window.require) {
                  return new Promise(function (resolve) {
                  ...
                  return new Promise(function (resolve) {
                      ...
                      scriptTag.onload = function () { return resolve(window[globalName]); };
          ...
          PictureparkConfig.get = function (element) {
              ...
              for (var i = 0; i < element.attributes.length; i++) {
                  ...
                  if (attribute.name === 'data-token')
                      ...
                      configuration[attribute.name.substr(5).replace(/-([a-z])/g, function (g) { return g[1].toUpperCase(); })] = attribute.value;
              }
  ...
  define("index", ["require", "exports", "templates", "players", "rendering", "config", "players"], function (require, exports, templates_1, players_2, rendering_1, config_1, players_3) {
      ...
      function processScriptTag(scriptTag) {
          ...
          var config = config_1.PictureparkConfig.get(scriptTag);
          ...
          return window.fetch(config.server + '/Service/PublicAccess/GetShare?token=' + config.token).then(function (response) {
              ...
              var share = {
                  ...
                  items: rawShare.contentSelections.map(function (s) {
                      var outputs = s.outputs.map(function (o) {
                          var embedItem = rawShare.embedContentItems.filter(function (e) { return e.contentId === o.contentId && e.outputFormatId === o.outputFormatId; })[0];
                          return {
                      ...
                      return {
                          ...
                          isMovie: originalOutput ? players_2.PictureparkPlayers.videoExtensions.indexOf(originalOutput.fileExtension) !== -1 : null,
                          isImage: originalOutput ? players_2.PictureparkPlayers.imageExtensions.indexOf(originalOutput.fileExtension) !== -1 : null,
              ...
              return engine.parseAndRender(contentTemplate, {
                  ...
                  var _loop_3 = function (item) {
                      if (item.isMovie) {
                          var itemId_1 = item.id;
                          var elementId_1 = 'player_' + item.index + "_" + id;
                          setTimeout(function () {
                              if (document.getElementById(elementId_1)) {
                                  players_2.PictureparkPlayers.renderVideoPlayer(config.token, itemId_1, elementId_1, config.width, config.height);
                              }
                          });
                      }
                  };
